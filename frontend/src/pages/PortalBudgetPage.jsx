...
import { budgetAPI, explanationsAPI, suggestionsAPI, ministryAPI, analyticsAPI } from '../services/api';
import { getCurrentMonth, getLast12Months, formatHebrewDate } from '../utils/format';
...
export default function PortalBudgetPage() {
  ...
  const parsedMunicipalityFromUrl = municipalityParam ? parseInt(municipalityParam, 10) : null;
  const municipalityFromUrl = Number.isFinite(parsedMunicipalityFromUrl) ? parsedMunicipalityFromUrl : null;
  const hasMonthInQuery = searchParams.has('month');

  const [budget, setBudget] = useState(null);
  const [selectedMonth, setSelectedMonth] = useState(initialMonth || '');
  const [selectedMunicipality, setSelectedMunicipality] = useState(municipalityFromUrl || user?.municipality_id || null);
  ...
  const [ministryCodeExists, setMinistryCodeExists] = useState(new Set());
  ...
  const [studentCountDeltas, setStudentCountDeltas] = useState({}); // keyed by topic_code
  ...
  // Update selectedMunicipality when URL or user changes
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const queryMuni = searchParams.get('municipality');
    if (queryMuni === String(selectedMunicipality)) {
      return;
    }

    const nextParams = new URLSearchParams(searchParams);
    nextParams.set('municipality', String(selectedMunicipality));
    setSearchParams(nextParams, { replace: true });
  }, [selectedMunicipality, searchParams, setSearchParams]);

  useEffect(() => {
    if (selectedMonth || !selectedMunicipality) {
      return;
    }
    ...
    const resolveDefaultMonth = async () => {
      try {
        const historyRes = await budgetAPI.getBudgetHistory(selectedMunicipality, 12);
        const availableMonths = Object.keys(historyRes?.data?.months || {}).sort().reverse();
        const fallbackMonth = availableMonths[0] || getCurrentMonth();
  ...
  }, [selectedMonth, selectedMunicipality, searchParams, setSearchParams]);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const budgetRes = await budgetAPI.getBudgetMonth(selectedMunicipality, selectedMonth);
      setBudget(budgetRes.data);

      // Get explanations including smart explanations
      try {
        const codesRes = await ministryAPI.getCodes();
        setMinistryCodeExists(new Set((codesRes.data || []).map(c => String(c.code))));
        const categoryMap = new Map((codesRes.data || []).map(c => [String(c.code), c.category || 'כללי']));
        setMinistryCodeCategory(categoryMap);
      } catch {
        setMinistryCodeExists(new Set());
        setMinistryCodeCategory(new Map());
      }

      // Lazy-load student-count deltas for the current run (non-fatal on failure).
      try {
        if (budgetRes.data?.run_id) {
          const deltasRes = await budgetAPI.getStudentCountDeltas(budgetRes.data.run_id, selectedMunicipality);
          const map = {};
          (deltasRes.data?.lines || []).forEach((row) => {
            map[String(row.topic_code)] = row;
          });
          setStudentCountDeltas(map);
        } else {
      ...
      try {
        const expRes = await explanationsAPI.getMonthExplanations(selectedMunicipality, selectedMonth);
        const expMap = {};
        const smartExpMap = {};
        
        if (expRes.data && typeof expRes.data === 'object') {
          // API returns { municipality_id, month, explanations: [...] }
          const explanations = expRes.data.explanations || [];
          if (Array.isArray(explanations)) {
            explanations.forEach(exp => {
              if (exp.topic_code && exp.explanation) {
                expMap[exp.topic_code] = exp.explanation;  // Use 'explanation' field, not 'custom_text'
              }
            });
          } else {
            // Fallback for old response format
            Object.keys(expRes.data).forEach(topicCode => {
              const explanation = expRes.data[topicCode];
              if (explanation && typeof explanation === 'object') {
                expMap[topicCode] = explanation.explanation || explanation.custom_text || explanation;
              } else if (typeof explanation === 'string') {
                expMap[topicCode] = explanation;
              }
            });
          }
        }
        setExplanations(expMap);
        setSmartExplanations(smartExpMap);
      } catch (err) {
        setExplanations({});
      ...
      if (err.response?.status === 404) {
        try {
          ...
          // Ignore history fallback errors and surface a user-friendly message below.
        }

        setError(hasMonthInQuery ? 'אין נתונים לחודש שנבחר.' : 'אין נתוני תקציב זמינים כרגע.');
        setBudget(null);
        setStudentCountDeltas({});
      } else {
  ...
  const handleExplanationSave = (topicCode, newText) => {
  ...
  const openSuggestionModal = (budgetLine) => {
  ...
  const handleSuggestionSubmit = async (suggestionData) => {
    if (!selectedMunicipality || !selectedBudgetLine) return;
    ...
    try {
      ...
      setSuccessMessage('✅ הצעתך נשלחה בהצלחה! המנהל יבדוק אותה בקרוב.');
  ...
  const toggleTopicInstitutionBreakdown = async (topicCode) => {
    ...
    try {
      const res = await budgetAPI.getTopicInstitutions(budget.run_id, selectedMunicipality, topicCode);
      setInstitutionByTopic((prev) => ({
        ...prev,
  ...
  // Phase 3.1: lazy-load the per-line detail for one topic and toggle visibility.
  // Returns BudgetLine rows grouped by line_type (gy / sharatim / mutavim /
  // yadaniim / moadon / sacal / mucarim / ...) with num_children,
  // participation_pct, and variance_driver metadata.
  const toggleTopicLinesDetail = async (topicCode) => {
    ...
    try {
      const res = await budgetAPI.getTopicLines(budget.run_id, selectedMunicipality, topicCode);
      setTopicLinesDetail((prev) => ({
        ...prev,
        [topicCode]: { loading: false, error: null, data: res.data, expanded: true },
      }));
  ...
  // Returns enrollment Δ, positions Δ, and rate Δ — showing what
  // actually drove the amount change (more kids, more FTE, or a
  // rate change).
  ...
  // Returns routes ranked by absolute Δ vs prior run, plus company /
  // vehicle-type summaries. Only called from transport topic rows.
  ...
  const toggleAllHighSchoolBreakdown = async () => {
    ...
    try {
      const res = await budgetAPI.getHighSchoolBreakdown(budget.run_id, selectedMunicipality);
  ...
  const getGroupColor = (lines) => {
    // Determine color based on line types and amounts
  ...
  // Phase 3.1: human-readable Hebrew labels for the raw line_type buckets
  // emitted by Phase 1 ingestion (gy, sharatim, mutavim, yadaniim, moadon,
  // sacal, mucarim, hasaot, shefi, cheshbonit).
  ...
  if (!selectedMonth) {
    return (
      <PortalWrapper title="פירוט תקציב" onBack={handleBack}>
  ...
  return (
    <PortalWrapper 
      title={`${user?.municipality_name || 'עיר'} — ${formatHebrewDate(selectedMonth)}`}
      onBack={handleBack}
    >
      <div className="w-full max-w-6xl mx-auto px-4">
        ...
        {budget && (
          <>
            ...
            {budget.month_changes?.has_changes && (
              <div className="bg-white rounded-2xl p-8 mb-10 shadow-lg border border-blue-200">
                <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
                  ...
                  <label className="inline-flex items-center gap-2 font-hebrew text-sm text-slate-700 cursor-pointer select-none">
                    <input
                      type="checkbox"
                      ...
                      className="h-4 w-4"
                    />
                    הצג רק שינויים ממספר ילדים
                  </label>
                </div>
                <div className="space-y-4">
                  {Object.entries(budget.month_changes.changes_by_topic)
                    .filter(([code]) => code !== '0')
                    .filter(([code]) => {
                      if (!onlyStudentCountChanges) return true;
                      return studentCountDeltas[String(code)]?.variance_driver === 'student_count';
                    })
                    .map(([code, change]) => (
                    <div key={code} className="bg-gradient-to-r from-blue-50 to-white border border-blue-100 rounded-xl p-5">
                      <div className="flex items-center justify-between gap-3 mb-3">
                        ...
                        <StudentCountDeltaChip delta={studentCountDeltas[String(code)]} />
                      </div>
                      ...
                      {(() => {
                        const explanation = generateChangeExplanation(change, explanations[code]);
                                ...
                                <div key={topicCode} className={`border-b border-slate-200 last:border-b-0 ${colors.bg}`}>
                                  ...
                                  >
                                    <div className="flex items-center gap-4 flex-1">
                                      ...
                                      <div className="flex-1">
                                        ...
                                        {ministryCodeExists.has(String(topicCode)) && (
                                          <button
                                            onClick={(e) => {
                                              e.stopPropagation();
                                              navigate(`/portal/ministry?code=${topicCode}`);
                                            }}
                                            className="mt-1 inline-flex items-center gap-1 px-2 py-0.5 bg-blue-100 hover:bg-blue-200 text-blue-700 rounded-full text-xs font-hebrew transition"
                                          >
                                            מידע ממשרד החינוך
                                          </button>
                                        )}
                                    ...
                                    <div className="text-right">
                                      ...
                                      {studentCountDeltas[String(topicCode)] && (
                                        <StudentCountDeltaChip delta={studentCountDeltas[String(topicCode)]} />
                                      )}
                                    </div>
                                  </div>
                                  ...
                                  {isExpanded && (
                                    <div className="px-8 py-6 space-y-4 border-t border-slate-200 bg-opacity-50">
                                            ...
                                            <div key={line.id} className={`p-4 rounded-lg border-l-4 ${lineColor}`}>
                                              <div className="flex items-center justify-between mb-2">
                                                ...
                                                <div className="flex items-center gap-3">
                                                  ...
                                                  <button
                                                    onClick={() => openSuggestionModal(line)}
                                                    title="הצע הסבר"
                                          ...
                                          <div className="mt-4 border-t border-slate-200 pt-4">
                                            ...
                                            {isDriversOpen && d && (
                                              <div className="mt-4 space-y-4">
                                                ...
                                                {/* Explained decomposition narrative */}
                                                {d.explained && (
                                                ...
                                                {/* Positions breakdown table (relevant only) */}
                                                ...
                                                {/* Enrollment breakdown table (top 10 movers) */}
                                      ...
                                      {/* Phase 3.3: Route-level transportation audit — only for topics 52 (regular) and 140 (special-ed). */}
      ...
      <ExplanationSuggestionModal
        ...
        topicCode={selectedBudgetLine?.topic_code}
